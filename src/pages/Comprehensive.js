...
let st = {}
function init() { st = { list:[], idx:0, correct:0, wrong:0, selectedWord:null } }

function buildWordQuestions() {
  return shuffle([...WORDS[1],...WORDS[2]]).slice(0,20).map(w => {
    const isKo = Math.random() > 0.5
    return { type:'fill-blank', _isWord:true,
      instruction: isKo ? '다음 한국어에 해당하는 영어 단어를 쓰시오.' : '다음 영어 단어의 뜻을 쓰시오.',
      sentence: isKo ? w.ko : w.en,
      answer: isKo ? w.en : w.ko,
      explanation: `${w.en} = ${w.ko}`, translation:'' }
  })
}

export function renderComprehensive(root) { init(); mount(root) }
...
function mount(root) {
  ...
  root.innerHTML = `
    <div style="display:flex;flex-direction:column;min-height:100vh">
      ...
      <div class="screen" style="padding-top:1rem">
        <div class="card" id="start-card" style="margin-bottom:1.2rem;display:flex;flex-direction:column;gap:.8rem">
          <div style="font-size:14px;color:var(--text2);line-height:1.7">
            단어 20문제 + 문법 전체를 랜덤으로 섞어서 출제함.<br>점수가 랭킹에 등록돼!
          </div>
  ...
  root.querySelector('#name-inp').addEventListener('input', e=>rankStore.set({playerName:e.target.value.trim()}))
  root.querySelector('#start-btn').addEventListener('click', ()=>startQuiz(root))
}

function startQuiz(root) {
  const wordQs = buildWordQuestions()
  const gramQs = shuffle([...(GRAMMAR_QUESTIONS[1]||[]),...(GRAMMAR_QUESTIONS[2]||[])])
  st.list = shuffle([...wordQs,...gramQs])
  st.idx=0; st.correct=0; st.wrong=0; st.selectedWord=null
  root.querySelector('#start-card').style.display='none'
  renderQ(root)
}
...
function renderQ(root) {
  ...
  if(st.idx >= st.list.length) {
    ...
    el.innerHTML = `
      <div class="result-screen">
        ...
        <div class="result-label">${st.correct}개 정답 / ${st.list.length}개 &nbsp;|&nbsp; 랭킹 등록 완료!</div>
  ...
  const passageHTML = q.passage ? `
    <div style="background:#13131f;border:1px solid #2a2a45;border-left:3px solid var(--purple);
  ...
  if(q.type==='error-correction') {
    body = `
      ...
      <div id="ec-input-area" style="display:none">
        ...
        <div style="display:flex;gap:8px">
          <input class="input" id="q-input" placeholder="고친 말 입력..." autocomplete="off" style="flex:1">
    ...
    if(q.sentence) body += `<div style="background:var(--surface2);padding:.7rem 1rem;border-radius:8px;
      font-size:${isWord?'1.2rem':'14px'};font-weight:${isWord?'700':'400'};
      margin-bottom:.8rem;line-height:1.8;text-align:${isWord?'center':'left'}">${q.sentence}</div>`
    body += `<input class="input" id="q-input" placeholder="정답 입력..." autocomplete="off">
...
function bindQ(root, q) {
  ...
  if(q.type==='error-correction') {
    el.querySelectorAll('.ec-token').forEach(token=>token.addEventListener('click',()=>{
      ...
      el.querySelector('#ec-input-area').style.display='block'
      el.querySelector('#ec-label').textContent=`"${token.dataset.word}"`
      const inp=el.querySelector('#q-input'); inp.value=''; inp.focus()
    }))
    const check=()=>{
      ...
      const inp=(el.querySelector('#q-input')?.value||'').trim().toLowerCase()
      const ok=(sel===wrong||wrong.includes(sel)||sel.includes(wrong))&&(inp===corr||corr.includes(inp)||inp.includes(corr))
      el.querySelector('#q-input').disabled=true; el.querySelector('#q-check').disabled=true
      showFb(ok)
    }
    el.querySelector('#q-input')?.addEventListener('keydown',e=>{if(e.key==='Enter')check()})
